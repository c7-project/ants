...
def scoreboard(screen, clock, ant_list):
    ...
    # Ants on ground
    display_text(screen, str(
        len(ant_list)), 14, (x_left, y_offset + 40))
    # Ants underground
    display_text(screen, str(
        ant_class.ants_underground), 14, (x_left, y_offset + 80))
    # Number of holes
    display_text(screen, str(
        len(hole_class.hole_locations)), 14, (x_left, y_offset + 120))
    # Number of rocks
    display_text(screen, str(
        rock_class.number_of_rocks), 14, (x_left, y_offset + 160))
    # Active sugar
    display_text(screen, str(
        sugar_class.active_sugar), 14, (x_left, y_offset + 200))
    # Sugar found
    display_text(screen, str(
        sugar_class.eaten_sugar), 14, (x_left, y_offset + 240))
